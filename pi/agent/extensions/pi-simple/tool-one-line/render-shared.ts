	...
	component.addChild(
		previewComponent(
			output
				...
				.join("\n"),
			context,
			isExpanded ? Number.POSITIVE_INFINITY : 10,
			"  ",
			leadingBlankLine,
			isExpanded ? undefined : (hiddenLineCount) => `${theme.fg("dim", `  ... ${hiddenLineCount} more lines`)} ${expandHint(theme)}`,
			false,
			!isExpanded,
		),
	);
...
export function renderCollapsedBashResult(
	...
	const outputLineCount = countBashOutputLines(result);
	const outputSummary = outputLineCount > 0
		? `${NBSP}${theme.fg("muted", `${outputLineCount} output ${outputLineCount === 1 ? "line" : "lines"}`)} ${expandHint(theme)}`
		: "";
...
export function renderPartialBashResult(
	...
	const component = resetContainer(context);
	addBashOutputPreview(component, result, theme, context, true);
	return component;
}
...
export function registerIntentTool(
	...
	renderPartialResult?: CollapsedIntentResultRenderer,
): void {
	pi.registerTool({
		...getBuiltInTool(process.cwd(), toolName),
		...
		renderResult(result: any, options: any, theme: any, context: any) {
			...
			const rawContext = context as any;
			const label = buildLabel(rawContext.args, toolTheme, rawContext.cwd);
			if (options.isPartial && renderPartialResult) {
...
export function renderBuiltInToolResult<TName extends BuiltInToolName>(
	...
	if (toolName === "read" && isFullFileRead(context.args)) {
		return renderMutedFullReadResult(result, options, theme as Theme, context);
	}
	return getBuiltInTool(cwd, toolName).renderResult?.(result as any, options as any, theme as any, context as any);
}

function isFullFileRead(args: { offset?: unknown; limit?: unknown } | undefined): boolean {
	return args?.offset === undefined && args?.limit === undefined;
}

function renderMutedFullReadResult(
	result: ToolResult,
	options: ToolRenderOptions,
	theme: Theme,
	context: RenderContext,
	...
	const output = getTextToolOutput(result, context.showImages);
	const lines = trimTrailingEmptyLines(output.replace(/\t/g, "    ").split("\n"));
	...
	const truncation = (result.details as any)?.truncation as {
		truncated?: boolean;
	...
	if (truncation?.truncated) {
		if (truncation.firstLineExceedsLimit) {
			text += `\n${theme.fg("warning", `[First line exceeds ${formatBytes(truncation.maxBytes ?? DEFAULT_READ_MAX_BYTES)} limit]`)}`;
		} else if (truncation.truncatedBy === "lines") {
			text += `\n${theme.fg("warning", `[Truncated: showing ${truncation.outputLines} of ${truncation.totalLines} lines (${truncation.maxLines ?? DEFAULT_READ_MAX_LINES} line limit)]`)}`;
		} else {
			text += `\n${theme.fg("warning", `[Truncated: ${truncation.outputLines} lines shown (${formatBytes(truncation.maxBytes ?? DEFAULT_READ_MAX_BYTES)} limit)]`)}`;
		}
	}
...
function getTextToolOutput(result: ToolResult, showImages: boolean): string {
	return (result.content ?? [])
		.flatMap((part) => {
			if (part?.type === "text") return [part.text ?? ""];
			if (part?.type === "image" && showImages) return [`[Image: ${part.mimeType ?? "unknown type"}]`];
			return [];
		})
		.join("\n");
}
...
export function countBashOutputLines(result: ToolResult<BashToolDetails>): number {
	const output = cleanBashOutput(result);
...
export function expandHint(theme: Theme): string {
	...
	return `${theme.fg("muted", "(")}${theme.fg("dim", expandKey)}${theme.fg("muted", " to expand")}${theme.fg("muted", ")")}`;
}
